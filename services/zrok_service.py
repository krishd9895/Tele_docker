...
Self-hosted zrok setup:
  Token comes from YOUR server, not zrok.io:
  ...
  If ZROK_API_ENDPOINT is set, enrollment points at your controller, not zrok.io.
"""
...
# zrok install — pulls the official binary, works for self-hosted instances too
...
class ZrokTunnelEngine:
    ...
    async def is_enrolled(self) -> bool:
        """Check if zrok has been enrolled on this host."""
    ...
    async def install_zrok(self) -> tuple[bool, str]:
        """Download and install the zrok binary on the WSL host."""
    ...
    async def enroll_zrok(self, token: str) -> tuple[bool, str]:
        ...
        If ZROK_API_ENDPOINT is set, points at the self-hosted controller.
        Token comes from:
          docker compose exec zrok-controller zrok admin create account email pass
          → look for accountToken in the output
        """
        token = token.strip()

        # Build the enable command — point at self-hosted controller if configured
    ...
    async def auto_enroll_if_needed(self) -> None:
        """
        Called at bot startup. If ZROK_PRIVATE_TOKEN is set in .env and
        zrok is not yet enrolled, enroll automatically — no user interaction needed.
        """
        ...
        if not await self.is_installed():
            logger.warning("ZROK_PRIVATE_TOKEN is set but zrok is not installed. Run /zrok_setup → Install.")
            return
        ...
        logger.info("Auto-enrolling zrok with ZROK_PRIVATE_TOKEN from .env...")
        success, out = await self.enroll_zrok(token)
        if success:
            logger.info(f"zrok auto-enrollment successful: {out[:80]}")
        else:
    ...
    async def create_share(
        ...
        # Read stdout until a public URL appears or timeout
        public_url = ""
        try:
            ...
            while loop.time() < deadline:
                ...
                if match:
                    ...
                    # Skip lines that are clearly not a share URL
